...
export default function PortalPage() {
  ...
  async function loadData(currUser: any) {
    const email = currUser.email.toLowerCase();
    const uid = currUser.id;

    const { data: buyer } = await sb.from(T.buyers).select('*')
      .or(`user_id.eq.${uid},email.ilike.%${email}%`).maybeSingle();

    const { data: app } = await sb.from(T.applications).select('*')
      .or(`user_id.eq.${uid},email.ilike.%${email}%`)
      .order("created_at", { ascending: false }).limit(1).maybeSingle();

    let puppy = null;
    if (buyer?.id) {
      const { data: p } = await sb.from(T.puppies).select('*').eq("buyer_id", buyer.id).maybeSingle();
      puppy = p;
    }

    const { data: msgs } = await sb.from(T.messages).select('*')
      .or(`user_id.eq.${uid},email.ilike.%${email}%`)
      .order("created_at", { ascending: false }).limit(5);

    setData({ buyer, app, puppy, msgs: msgs || [] });
  }

  if (loading) return <div className="h-screen flex items-center justify-center bg-brand-50 italic">Loading Portal...</div>;
  ...
  return (
    <div className="flex h-screen overflow-hidden text-brand-900 bg-brand-50">
      ...
      {/* MOBILE DRAWER */}
      <aside className={`fixed top-0 left-0 bottom-0 w-[82%] max-w-[320px] bg-[#FDFBF9] z-50 shadow-2xl flex flex-col transition-transform duration-300 md:hidden ${isDrawerOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <aside className="hidden md:flex flex-col w-72 bg-white/80 border-r border-brand-200/60 z-20 h-full backdrop-blur-sm">
        <div className="p-8">
          ...
          <p className="text-[10px] uppercase tracking-widest text-brand-500 font-black mt-1">Chihuahua</p>
        </div>
        <nav className="flex-1 px-4 pt-6 space-y-3">
          <div className="px-4 py-2 text-[10px] font-black uppercase text-brand-400">Portal</div>
          <a href="#" className="nav-item active">Dashboard</a>
          <a href="#" className="nav-item">Application</a>
          <a href="#" className="nav-item">My Puppy</a>
        </nav>
        <div className="p-4 border-t border-brand-100 bg-brand-50/50">
          <p className="text-xs font-black truncate">{user.email}</p>
          <button onClick={() => sb.auth.signOut()} className="text-[10px] font-black uppercase text-brand-500 hover:text-brand-800">Sign Out</button>
        </div>
      </aside>
        ...
        <div className="flex-1 p-6 md:p-12 overflow-y-auto scroller max-w-[1600px] mx-auto w-full">
          ...
          <div className="mb-10">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/70 border border-brand-200 shadow-paper mb-4">
              <span className="text-[10px] font-black uppercase tracking-[0.22em] text-brand-500">My Puppy Portal</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
            <div className="lg:col-span-2 card-luxury shine p-8 relative overflow-hidden">
              <h3 className="font-serif text-3xl font-bold mb-2">
                {data?.puppy ? `Match Confirmed: ${data.puppy.call_name}` : "Application Received"}
              </h3>
              <p className="text-brand-600 font-semibold mb-6">Review documents and messages for next steps.</p>
              <div className="flex flex-wrap gap-3">
                <button className="px-7 py-3.5 bg-brand-800 text-white font-black text-sm rounded-xl uppercase tracking-wider">Open Application</button>
                <button className="px-7 py-3.5 bg-white border border-brand-200 text-brand-800 font-black text-sm rounded-xl uppercase tracking-wider">Message Support</button>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8">
              {data?.puppy ? (
                <div className="card-luxury overflow-hidden h-[450px] relative group">
                  <img src={buildPuppyPhotoUrl(data.puppy.image_url)} className="w-full h-full object-cover transition duration-700 group-hover:scale-105" alt="Puppy" />
                  <div className="absolute bottom-0 left-0 p-8 bg-gradient-to-t from-black/70 to-transparent w-full">
                    <h3 className="text-white font-serif text-5xl font-bold">{data.puppy.call_name}</h3>
                    <p className="text-white/80 font-semibold">{data.puppy.sex} • Born {fmtDate(data.puppy.dob)}</p>
                  </div>
                </div>
              ) : (
                <div className="card-luxury p-16 text-center border-dashed border-2 border-brand-200 bg-white/40">
                  ...
                  <p className="text-brand-500 mt-2">Updates appear after application approval and matching.</p>
                </div>
              )}
            </div>
            ...
            <div className="lg:col-span-4">
              <div className="card-luxury p-7 h-full">
                <h4 className="font-serif font-bold text-2xl text-brand-800 mb-6">Recent Messages</h4>
                <div className="space-y-4">
                  {data?.msgs.length > 0 ? data.msgs.map((m: any) => (
                    <div key={m.id} className="p-4 bg-brand-50/50 rounded-xl border border-brand-100">
                      <div className="flex justify-between mb-1">
                        <span className="text-[10px] font-black text-brand-500 uppercase">{m.sender || 'Support'}</span>
                        <span className="text-[10px] text-brand-300 font-semibold">{fmtDate(m.created_at)}</span>
                      </div>
                      <p className="text-sm font-semibold text-brand-800 line-clamp-2">{m.message}</p>
                    </div>
                  )) : (
                    <p className="text-center py-8 text-brand-400 italic">No new messages</p>
                  )}
                </div>
              </div>
            </div>
      ...
      {/* FLOATING ACTION */}
      <a href="#" className="fixed bottom-6 right-6 w-14 h-14 bg-brand-800 text-white rounded-full shadow-luxury flex items-center justify-center hover:scale-105 transition z-50">
    ...
    <div className="h-screen flex items-center justify-center bg-brand-50 p-6">
      <div className="card-luxury shine p-10 w-full max-w-md border border-white">
        ...
        <form onSubmit={login} className="space-y-5">
          ...
          <button className="w-full bg-brand-800 text-white p-4 rounded-xl font-black uppercase text-xs tracking-widest shadow-lift">Sign In</button>
        </form>
      </div>
    </div>
  );
