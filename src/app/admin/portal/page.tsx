...
export default function AdminPortalPage() {
  ...
  const availablePuppies =
    workspace?.summary.availableCount ?? stats.lineage?.availablePuppies ?? 0;
  const reservedPuppies =
    workspace?.summary.reservedCount ?? stats.lineage?.reservedPuppies ?? 0;
  const completedPuppies =
    workspace?.summary.completedCount ?? stats.lineage?.completedPuppies ?? 0;
  ...
  const topDogs = (workspace?.dogs || [])
    ...
    .sort((left, right) => right.summary.realizedRevenue - left.summary.realizedRevenue)
    .slice(0, 6);
  const latestDigest = stats.latestDigest;
  const queueRows = [
    {
      href: "/admin/portal/users",
      label: "Buyers in system",
    ...
    },
    {
      ...
      label: "Applications awaiting flow",
    ...
    },
    {
      ...
      label: "Litters in program",
      value: String(totalLitters),
      detail: `${availablePuppies} puppies still available`,
      status: totalLitters > 0 ? "active" : "pending",
    },
    {
      href: "/admin/portal/dams-sires",
      label: "Breeding dogs tracked",
      value: String(totalDams + totalSires),
      detail: `${totalDams} dams and ${totalSires} sires`,
    ...
    <AdminPageShell>
      <div className="space-y-5 pb-10">
        <AdminPageHero
          ...
          description="The admin is organized around the real operating model now: buyers, litters, breeding dogs, puppies, financing, payments, documents, and message follow-up. This view stays compact, scannable, and connected to the live data model."
          actions={
            <>
          ...
          aside={
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
              <AdminInfoTile
                ...
                value={fmtMoney(realizedRevenue)}
                detail="Completed and buyer-linked revenue tracked internally across litters, breeding dogs, buyers, and payments."
              />
              <AdminInfoTile
                label="Projected Revenue"
                value={fmtMoney(projectedRevenue)}
                detail="List and sale values still in play across available and reserved puppies."
              />
              <AdminInfoTile
        ...
        <AdminMetricGrid>
          <AdminMetricCard
            label="Buyers"
            value={String(stats.buyers)}
            detail="Active buyer records and completed accounts managed in one workspace."
          />
          <AdminMetricCard
            label="Applications"
            value={String(stats.applications)}
            detail="Application flow and review queue waiting on decisions or follow-up."
            accent="from-[#f0ddc5] via-[#d9b78e] to-[#be8650]"
          />
          <AdminMetricCard
            label="Litters"
            value={String(totalLitters)}
            detail="First-class litter records with linked dams, sires, puppies, and revenue."
        ...
        <section className="grid grid-cols-1 gap-5 2xl:grid-cols-[minmax(0,1.25fr)_420px]">
          <div className="space-y-5">
            <AdminPanel
              ...
              subtitle="Instead of oversized summary cards, the admin now exposes the live queue by operating area."
            >
                      ...
                      <tr key={row.label} className="hover:bg-[#fffaf4]">
                        ...
                        <td className="px-4 py-3">
                          <div className="inline-flex items-center gap-2">
                            <span className="text-base font-semibold text-[#2f2218]">{row.value}</span>
                            <span
            ...
            <div className="grid gap-5 xl:grid-cols-2">
              <AdminPanel
                ...
                subtitle="Top litters ranked by realized revenue with direct lineage context."
              >
                {topLitters.length ? (
                  <div className="overflow-hidden rounded-[24px] border border-[#ead9c7]">
                    <table className="min-w-full divide-y divide-[#eee1d2] text-sm">
                          ...
                          <tr key={litter.id} className="hover:bg-[#fffaf4]">
                            <td className="px-4 py-3">
                              ...
                              <div className="mt-1 text-xs text-[#8a6a49]">
                                {litter.whelp_date ? fmtDate(litter.whelp_date) : "Whelp date not set"} •{" "}
                                {puppyLine(litter)}
                              </div>
                            </td>
                            <td className="px-4 py-3 text-[#73583f]">
                              {(litter.damProfile?.displayName || "No dam") + " / " + (litter.sireProfile?.displayName || "No sire")}
                            </td>
                            <td className="px-4 py-3">
                              ...
                              <div className="mt-1 text-xs text-[#8a6a49]">
                                {litter.summary.completedCount} completed • {litter.summary.reservedCount} reserved
                              </div>
                            </td>
                          </tr>
                        ))}
              ...
              <AdminPanel
                title="Dam & Sire Performance"
                subtitle="A direct read on breeding output, completion rate, and realized revenue."
              >
                {topDogs.length ? (
                    ...
                    {topDogs.map((dog) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <div className="text-sm font-semibold text-[#2f2218]">{dog.displayName}</div>
                            <div className="mt-1 text-xs leading-5 text-[#8a6a49]">
                              {dogLabel(dog)} • {dog.summary.totalLitters} litters • {dog.summary.totalPuppies} puppies
                            </div>
                          </div>
                          <span
                            className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] ${adminStatusBadge(
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2">
                          ...
                          <MiniSnapshot
                            label="Complete"
                            value={`${Math.round(dog.summary.completionRate * 100)}%`}
                          />
                          <MiniSnapshot
                            label="Reserved"
                            value={`${Math.round(dog.summary.reserveRate * 100)}%`}
                          />
                        </div>
                      </div>
                    ))}
          ...
          <div className="space-y-5">
            <AdminPanel
              ...
              subtitle="Daily pressure points, conversation flow, and links into the next action."
            >
              <div className="grid gap-3">
                <AdminInfoTile
                  label="Latest Digest"
                  value={latestDigest ? fmtDate(latestDigest.digest_date) : "No brief yet"}
                  detail={latestDigest?.summary || "Run the daily digest to surface summarized priorities here."}
                />
                <AdminInfoTile
            ...
            <AdminPanel
              ...
              subtitle="A compact view of the newest buyer threads so the overview still feels operational."
            >
