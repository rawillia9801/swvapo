...
import { SUPABASE_ANON_KEY, SUPABASE_URL } from "@/lib/utils";
  ...
  try {
    return await Promise.race([
      supabase
        .from("bp_dogs")
        .select(
          "dog_name, call_name, role, sex, color, coat, registry, is_active",
        )
        .order("dog_name", { ascending: true }),
      new Promise<{ data: DogRow[] | null; error: { message: string } }>(
        (resolve) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {dogs?.map((dog, index) => {
            ...
            return (
              <div
                key={index}
                className="group relative bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl"
              >
                <div className="relative h-80 bg-slate-100 overflow-hidden border-b border-slate-50 flex items-center justify-center">
                  ...
                  <span
                    className={`absolute bottom-6 left-8 px-5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-lg ${isSire ? "bg-blue-600 text-white border-blue-500" : "bg-rose-50 text-white border-rose-400"}`}
                  >
                    {dog.role}
                  </span>
                </div>
                ...
                <div className="p-10 pt-8">
                  ...
                  <button
                    type="button"
                    disabled
                    title="Full public pedigree profiles have not been published yet."
                    className="w-full cursor-not-allowed rounded-2xl border border-slate-200 bg-slate-100 py-5 text-center text-[11px] font-black uppercase tracking-[0.16em] text-slate-500"
                  >
                    Pedigree profile not yet published
                  </button>
                </div>
              </div>
            );
