...
function LedgerStatCard({
  ...
  return (
    <div className="rounded-[20px] border border-[var(--portal-border)] bg-white px-4 py-4 shadow-sm">
      <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--portal-text-muted)]">
        {label}
      </div>
      <div className="mt-2 text-lg font-semibold text-[var(--portal-text)]">{value}</div>
...
function LedgerEntryCard({ entry }: { entry: LedgerEntry }) {
  const hasCharge = entry.charge > 0;
  const hasCredit = entry.credit > 0;

  return (
    <div className="rounded-[24px] border border-[var(--portal-border)] bg-[linear-gradient(180deg,rgba(255,255,255,1)_0%,rgba(249,249,247,1)_100%)] p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-2 text-sm leading-7 text-[var(--portal-text-soft)]">{entry.detail}</div>
        </div>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        <div className="rounded-[18px] border border-[var(--portal-border)] bg-[var(--portal-surface-muted)] px-4 py-3">
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--portal-text-muted)]">
            Charge
          </div>
          <div className="mt-2 text-base font-semibold text-[var(--portal-text)]">
            {hasCharge ? toMoney(entry.charge) : "—"}
          </div>
        </div>
        ...
        <div className="rounded-[18px] border border-[var(--portal-border)] bg-[var(--portal-surface-muted)] px-4 py-3">
          ...
          <div className="mt-2 text-base font-semibold text-[var(--portal-text)]">
            {hasCredit ? toMoney(entry.credit) : "—"}
          </div>
        </div>

        <div className="rounded-[18px] border border-[var(--portal-border)] bg-white px-4 py-3 shadow-sm">
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--portal-text-muted)]">
            Running Balance
          </div>
          <div className="mt-2 text-base font-semibold text-[var(--portal-text)]">
...
export default function PortalPaymentsPage() {
  ...
  const recentEntries = useMemo(() => [...entries].slice().reverse().slice(0, 5), [entries]);
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.05fr)_430px]">
        <div className="space-y-6">
          ...
          <PortalPanel
            ...
            subtitle="A premium billing-style view of how the balance was built, reduced, and where it stands now."
          >
            {entries.length ? (
              <div className="space-y-5">
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                  <LedgerStatCard
                    ...
                    detail="The balance remaining on this account right now."
                  />
                  <LedgerStatCard
                    label="Charges Posted"
                    value={toMoney(summary.totalCharges)}
                    detail="All charges currently reflected in the ledger."
                  />
                  <LedgerStatCard
                    label="Credits Applied"
                    value={toMoney(summary.totalCredits)}
                    detail="Payments and credits that reduced the balance."
                  />
                  <LedgerStatCard
                    label="Ledger Entries"
                    value={String(entries.length)}
                    detail="Every recorded charge, credit, and payment."
                  />
                </div>

                <div className="rounded-[24px] border border-[var(--portal-border)] bg-[linear-gradient(180deg,rgba(255,255,255,1)_0%,rgba(247,249,252,1)_100%)] p-5 shadow-sm">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--portal-text-muted)]">
                      ...
                      <div className="mt-2 text-base font-semibold text-[var(--portal-text)]">
                        Most recent ledger activity
                      </div>
                    </div>
                    <div className="rounded-full border border-[var(--portal-border)] bg-white px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--portal-text-muted)]">
                  ...
                  <div className="mt-5 space-y-3">
                    {recentEntries.map((entry) => (
                      <LedgerEntryCard key={entry.key} entry={entry} />
                    ))}
                  </div>
                </div>
