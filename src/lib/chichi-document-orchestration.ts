...
function isForceRequest(rawMessage: string | null | undefined) {
  ...
  return (
    ...
    text.includes("replace signed copy") ||
...
function shouldOverrideActiveFlow(
  ...
  if (current.active_flow === "zoho_forms" && nextFlow === "zoho_writer_sign") {
    return !["sent_to_sign", "signed", "filed"].includes(currentStatus);
  }
...
async function resolveBuyer(admin: SupabaseClient, options: ResolveOptions) {
...
async function loadForms(admin: SupabaseClient, user: User, buyer: BuyerRow | null) {
...
function buildPreparedText(
  ...
  const effectiveStatusDetail = options?.effectiveStatusDetail || null;

  const lines = [
    force
      ? `ChiChi forced a new ${item.definition.title} for ${buyerDisplayName(context.buyer, context.user)}.`
      : `ChiChi prepared the ${item.definition.title} for ${buyerDisplayName(context.buyer, context.user)}.`,
    "",
    force
      ? `${item.note} Admin override was requested, so ChiChi ignored the previous completed-signature state and launched a fresh send path.`
      : item.note,
    "",
    `Status: ${item.status.label}`,
    effectiveStatusDetail || item.status.detail,
  ];
  ...
  lines.push(
    ...
    "Workflow:",
    force
      ? "1. ChiChi reset the active send state for this package without deleting historical records."
...
async function launchPreparedPackage(
  ...
  if (force) {
    workflow = mergeChiChiDocumentPackageWorkflow(workflow, {
      active_flow: item.flow,
      package_status: item.flow === "zoho_forms" ? "awaiting_buyer_input" : "prepared",
      signed_at: null,
      filed_at: null,
      sent_to_sign_at: null,
      review_note: "Admin override requested a fresh send for this package.",
      launch_url:
        item.flow === "zoho_forms"
          ? launches.zohoFormsUrl || launches.portalReviewUrl
          : item.flow === "zoho_writer_sign"
            ? launches.zohoSignUrl || launches.portalReviewUrl
      ...
      zoho: {
        ...
        sign_embed_url: null,
        sign_completed_document_url: null,
        sign_completed_document_name: null,
      },
    ...
    const resetSubmission = await updatePackageSubmissionWorkflow(
      admin,
      submission,
      item.prefill,
      workflow,
    ...
    submission = resetSubmission || submission;
  }
  ...
  if (item.flow === "zoho_writer_sign") {
    ...
    if (!workflow.zoho?.sign_request_id || force) {
      const zohoLaunch = await createZohoWriterSignRequest({
        ...
        privateNotes: force
          ? "Force resend triggered by admin override."
          : item.note,
      });
      ...
      workflow = mergeChiChiDocumentPackageWorkflow(workflow, {
        ...
        last_synced_at: new Date().toISOString(),
        review_note: force
          ? "Admin override requested a new Zoho Sign send."
          : workflow.review_note || null,
        launch_url:
  ...
  if (item.flow === "zoho_forms") {
    workflow = mergeChiChiDocumentPackageWorkflow(workflow, {
      ...
      last_synced_at: new Date().toISOString(),
      review_note: force
        ? "Admin override requested a fresh buyer-input cycle."
        : workflow.review_note || null,
    });
  ...
  workflow = mergeChiChiDocumentPackageWorkflow(workflow, {
    ...
    last_synced_at: new Date().toISOString(),
    review_note: force
      ? "Admin override requested a fresh native portal send."
      : workflow.review_note || null,
  });
...
export async function prepareChiChiDocumentPackage(
  ...
  const effectiveItem =
    force && (item.status.phase === "filed" || item.status.phase === "signed")
      ? {
          ...item,
          signedCopy: null,
          workflow: mergeChiChiDocumentPackageWorkflow(item.workflow, {
          ...
          status: {
            ...
            phase: "prepared",
            label: "Prepared",
            detail: "Admin override triggered a fresh resend and ignored the prior completed-signature state.",
            complete: false,
          },
        }
      : item;
  ...
  return {
    text: buildPreparedText(item, context, launches, {
      ...
      effectiveStatusDetail:
        force && launched.workflow?.package_status === "sent_to_sign"
          ? "Admin override triggered a fresh Zoho Sign send for this package."
          : force && launched.workflow?.package_status === "awaiting_buyer_input"
            ? "Admin override triggered a fresh buyer-input cycle for this package."
            : force && launched.workflow?.package_status === "fallback_portal_flow"
              ? "Admin override triggered a fresh native portal review path for this package."
              : null,
    }),
